"""Validate the corners-only enclosing-cone metric for ISEA7H (DGGAL).
...
levels, so this script checks empirically: for a sample of cells across
resolutions — including the coarsest levels and the 12 pentagons (level 0) —
it compares the aspect ratio from corners against the ratio from edge-refined
vertices (`getZoneRefinedWGS84Vertices`, which densifies each edge). If the max
delta is within solver tolerance, corners-only is confirmed and we keep it.
...
ad = dggal_common.Adapter('ISEA7H')


def refined_vec3(zone, refine):
...
def main():
    ...
    print(f'ISEA7H corners-vs-refined (edgeRefinement={REFINE})')
    print(f'{"lvl":>3} {"cells":>6} {"pents":>6} {"max|dAR|":>10} '
          f'{"max_rel":>10} {"corners_AR_range":>22}')
    overall = 0.0
    for level in LEVELS:
        zones = cells_for_level(level, rng)
        npent = 0
        max_abs = max_rel = 0.0
        ars = []
        for z in zones:
    ...
    print(f'\noverall max |dAR| = {overall:.3e}')
    print('corners-only CONFIRMED (within solver tolerance)' if overall < 1e-3
